...
def submit_job(prefix, model, pruner, sparsity, pretrain_epochs, fine_tune_epochs, dataset='cifar10'):
    ...
    - cd /nni/examples/model_compress \
    ...
    - python3 auto_pruners_torch.py \
      ...
      --data-dir /{dataset} \
      --experiment-data-dir /mnt/confignfs-data/znx/experiment_data/{dataset}/{model}/{pruner}/{sparsity_str}".format(
...
if __name__ == '__main__':
    ...
    pruners = ['AutoCompressPruner', 'NetAdaptPruner', 'SimulatedAnnealingPruner']
    sparsities = ['0.1', '0.3', '0.5', '0.7', '0.9', '0.95', '0.975']
    for model in models:
        for pruner in pruners:
            for sparsity in sparsities:
                submit_job('0804', model, pruner, sparsity, pretrain_epochs=200, fine_tune_epochs=200)