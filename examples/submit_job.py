...
def submit_job(prefix, model, pruner, sparsity, pretrain_epochs, fine_tune_epochs, dataset='cifar10', lr=0.001, constrained=True, load_checkpoint=True):
    ...
    - python3 -u auto_pruners_torch.py \
      ...
      --experiment-data-dir /mnt/confignfs-data/znx/experiment_data/{dataset}/{model}/{prefix}_{pruner}_{constrained}/{sparsity_str}".format(
          dataset=dataset, model=model, pruner=pruner, sparsity=sparsity, sparsity_str=str(sparsity).replace('.', ''), pretrain_epochs=pretrain_epochs, fine_tune_epochs=fine_tune_epochs, constrained=str(constrained), prefix=prefix, lr=lr, load_checkpoint=load_checkpoint)
            ...
            for sparsity in sparsities:
                ...
                for lr in LRs:
                    for constrained in [False]:
                        ...
                        submit_job('0824', model, pruner, sparsity, pretrain_epochs=0, fine_tune_epochs=200, dataset='cifar10', load_checkpoint=True, constrained=constrained)